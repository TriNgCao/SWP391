...
export default function StylistSchedule() {
  ...
  // Fetch stylist's schedule using accountId
  ...
  useEffect(() => {
    fetchStylistSchedule(); // Fetch the stylist's schedule on component mount
  }, []);

  // Select schedule to display detailed appointments in the modal
  const handleSelectDate = (date) => {
    const selectedSchedule = scheduleData.find((item) => item.date === date);
    if (selectedSchedule) {
      setSelectedDate(date);
