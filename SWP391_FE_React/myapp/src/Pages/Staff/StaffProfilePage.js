...
const StaffProfilePage = () => {
  ...
  const handleSubmit = async (values) => {
    try {
      ...
      // Giữ lại giá trị cũ của profile và chỉ cập nhật các trường có giá trị mới
      const updatedValues = {
        name: values.name || profile.name, // Nếu không có giá trị mới, giữ giá trị cũ
        phone: values.phone || profile.phone,
      };

      // Giữ nguyên email mà không gửi nó trong updatedValues
      updatedValues.email = profile.email; // Đảm bảo email không bị thay đổi
      ...
      // Cập nhật lại profile với các giá trị mới hoặc cũ
