...
export default function ManagerSchedule() {
    ...
    const fetchStylists = async (date) => {
        try {
            const response = await axios.get(
                `http://localhost:8080/schedule/view/stylist/MAN1`
            );
    ...
    const fetchSchedules = async (date) => {
        try {
            const response = await axios.get(
                `http://localhost:8080/schedule/view/MAN1/${date}`
            );
    ...
    const handleDeleteSchedule = async (stylistName, date) => {
        if (window.confirm("Are you sure you want to delete this schedule?")) {
            try {
                ...
                await axios.delete(`http://localhost:8080/schedule/delete/${encodedStylistName}/${encodedDate}`);
    ...
    const handleSaveStylists = async () => {
        const stylistIds = selectedStylists.map((stylistName) => {
            const stylist = stylistData.find((sty) => sty.stylistName === stylistName);
            console.log(stylist);
            ...
            <Box sx={{ padding: 4, backgroundColor: "#f9f9f9", minHeight: "100vh" }}>
                                    ...
                                    <TableRow key={index}>
                                        ...
                                        <TableCell>
                                            <IconButton
                                                ...
                                                onClick={() => handleDeleteSchedule(item.name, item.date)}
                ...
                <Modal open={addOpen} onClose={handleClose}>
                    ...
                    >
                        ...
                        >
                            Select Stylists for {selectedDate ? selectedDate.toLocaleDateString() : "Select a date"}
                        </Typography>
                                ...
                                <TableBody>
                                    {stylistData.map((stylist, index) => (
                                        <TableRow key={index}>
                                            ...
                                            <TableCell>
                                                <Checkbox
                                                    checked={selectedStylists.includes(stylist.stylistName)}
                                                    onChange={() => handleSelectStylist(stylist.stylistName)}
                                                />
                                            </TableCell>
                                        </TableRow>
                                    ))}
